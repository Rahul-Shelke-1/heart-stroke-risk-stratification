...
class DataValidation:
    ...
    def initiate_data_validation(self) -> DataValidationArtifact:
        ...
        try:
            ...
            if not status:
                validation_error_msg += (
                    "Numerical columns are missing in train dataframe."
                )
            status = self.is_numerical_column_exist(df=test_df)

            if not status:
                validation_error_msg += (
                    "Numerical columns are missing in test dataframe."
                )
            status = self.is_categorical_column_exist(df=train_df)

            if not status:
                validation_error_msg += (
                    "Categorical columns are missing in training dataframe."
                )
            status = self.is_categorical_column_exist(df=test_df)

            if not status:
                validation_error_msg += (
                    "Categorical columns are missing in test dataframe."
                )
